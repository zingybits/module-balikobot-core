...
/**
 * Class BalikobotApiClient
 * @package ZingyBits\BalikobotCore\Model
 */
class BalikobotApiClient implements BalikobotApiClientInterface
{
    ...
    public function __construct(
        ...
        if (empty($apiUser) || empty($apiKey) || empty($shopId))
            throw new \InvalidArgumentException('Balikobot API token and key are not in DB.');
        if (!is_int($shopId))
    ...
    public function add() {
        ...
        if (!isset($response[0]['package_id']))
            throw new \InvalidArgumentException('Invalid arguments. Errors: ' . json_encode($response[0]) . '.', self::EXCEPTION_INVALID_REQUEST);
